...
int main()
{
  ...
  string command;        // operation to be executed
    ...
    // all declared statically not pointers
    // instance that containes all three attributes
  ...
  { 
    ...
    else if (command == "GetItem")
    {
	  ...
	  list.MakeEmpty();
        ...
        // declares which command it will be
       		item.Initialize(2);
      		listToCount.PutItem(item);
      		item.Initialize(5);
    	    listToCount.PutItem(item);
	  ...
	  //     	listSplitTest.PutItem(item);
        ...
        // test cases to see if code runs correctly 
        item.Initialize(9);
        listSplitTest.PutItem(item);
        item.Initialize(5);
        listSplitTest.PutItem(item);

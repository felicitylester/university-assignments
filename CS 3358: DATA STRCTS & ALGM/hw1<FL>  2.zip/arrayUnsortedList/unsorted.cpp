...
{
    // Post: all items in list are successfully reversed
    NodeType* previous = nullptr;
    NodeType* current = listData;
    NodeType* next = nullptr;

    // nullptr indicates end of list
    while (current != nullptr) {
        // reads right side first
        // current = used to transverse over the data
        next = current->next;
    ...
    // listData is now pointing to the end of the reversed list
    // Move it to the beginning to restore the original order
    listData = previous;
}
